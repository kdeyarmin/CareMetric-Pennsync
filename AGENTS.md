...
`test:pennsync-import:postgres` needs a real PostgreSQL, is gated behind `PENNSYNC_TEST_PG_URL`, and is **not** in `pnpm test`, so a change to `pennsync_private` can pass every local check and fail only in CI. `record-store-migration.test.mjs` carries a PGlite equivalent of its inbound-key guard for that reason; run the postgres suite too when touching that schema.
