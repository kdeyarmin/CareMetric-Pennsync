...
**Eleven postgres-backed suites are not in `pnpm test`**, so a change to `pennsync_private` can pass lint, the whole suite, the build, the typecheck gate and all seven gates and still fail CI — which it did, twice, because the set was guessed at rather than enumerated. They all need `PENNSYNC_TEST_PG_URL`, and `restore-postgres` additionally needs `PENNSYNC_TEST_PG_BIN` plus real `pg_dump`/`pg_restore`:
...
services/authority-store/tests/{postgres,s4-postgres,visit-documentation-postgres,
  ...
  referral-list-postgres,s3-postgres,restore-postgres}.test.mjs
