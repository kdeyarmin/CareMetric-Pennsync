    ...
    <div className="md:hidden fixed top-0 left-0 right-0 z-50 bg-white shadow-sm border-b border-slate-200 print:hidden safe-top">
      ...
      <div className="flex items-center gap-1">
        ...
        <Button
          variant="ghost"
          size="icon"
          className="relative text-slate-500 hover:text-navy-700 hover:bg-slate-100 h-10 w-10"
          onClick={onOpenNotificationCenter}
          aria-label={notificationsAvailable ? 'Notifications' : 'Notifications unavailable'}
          title={notificationsAvailable ? 'Notifications' : 'Notifications unavailable pending tenant-safe access'}
          disabled={!notificationsAvailable}
        >
          <Bell className="w-5 h-5" />
          {notificationsAvailable && totalNotificationCount > 0 && (
