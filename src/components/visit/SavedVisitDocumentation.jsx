...
export default function SavedVisitDocumentation({ visit, patient, editingAvailable = true }) {
  ...
  return (
    <section className="space-y-5 rounded-xl border border-slate-200 bg-white p-5">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-600">{editingAvailable
          ? 'Read-only saved record. Use the editor below to prepare and review changes; viewing this record does not save or re-verify it.'
          : 'Read-only saved record. Viewing this record does not change or re-verify it.'}</p>
