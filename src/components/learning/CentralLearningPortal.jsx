...
export default function CentralLearningPortal({ authoring = false }) {
  return <section className="mx-auto my-8 max-w-4xl rounded-xl border bg-white p-8 shadow-sm" aria-label="CareMetric central learning">
    ...
    <div className="flex flex-wrap gap-4">
      <a className="inline-flex items-center gap-2 rounded-lg bg-teal-700 px-5 py-3 font-medium text-white" href={PENNSYNC_LEARNING_URL} target="_blank" rel="noopener noreferrer"><BookOpen size={18} /> Browse courses</a>
      <a className="inline-flex items-center gap-2 rounded-lg border px-5 py-3" href={CENTRAL_MY_LEARNING_URL} target="_blank" rel="noopener noreferrer"><GraduationCap size={18} /> My learning</a>
      {authoring && <a className="inline-flex items-center gap-2 rounded-lg border px-5 py-3" href={CENTRAL_COURSE_EDITOR_URL} target="_blank" rel="noopener noreferrer"><PenLine size={18} /> Create course</a>}
    </div>
  </section>;
}
