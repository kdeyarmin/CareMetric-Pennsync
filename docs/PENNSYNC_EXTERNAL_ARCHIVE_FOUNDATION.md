...
- `references`: objects with `pointer` and target `entity`. Targets must exist in the same source namespace. JSON Pointers support `*` for array elements, such as `/previous_versions/*/document_id`.
...
Focused validation: the executable tests cover exact-byte round trip through an independent decryptor; separate-source same-ID preservation; wrong key/corruption/truncation/header substitution; missing seal and extra frames; sealed resume and changed inputs; missing identity/file manifests; duplicate IDs; wrong-source/orphan references; ambiguous mappings; nested credentials; duplicate JSON keys; file owner/binding checks; empty reference arrays; bounded large-file frames; path/symlink refusal; and actual CLI stdin key delivery. This is synthetic evidence only.
