...
`rollback` deletes only the unchanged patients in that exact verified batch, under the same lock. Existing assignment/referral/visit foreign keys refuse deletion when dependent work exists; it never cascades. A terminal `rolled_back` receipt remains, so lost rollback acknowledgements can be reconciled and the old batch cannot silently be reimported. It does not undo unrelated Auth, agency, membership or source data. This model assumes trusted local migration administrators; it is not protection against a superuser forging receipts or disabling constraints.
...
There is no plaintext extraction directory, dump, row-valued stdout or raw provider-error logging. Only the intended synthetic business rows are stored in PostgreSQL. The connection disables statement/parameter error logging for this local session; external administrator-controlled logging is outside the tool's control. The source archive is retained byte-for-byte and must remain available with its recovery key for reconciliation/rollback. The importer never edits or removes it.
