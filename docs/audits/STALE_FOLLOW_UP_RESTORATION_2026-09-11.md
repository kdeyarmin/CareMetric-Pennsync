...
| Ordinary Referral edits could overwrite worker claim/finalization markers. | All five stale-notification fields are server-owned. Edits preserve them only for the same follow-up generation; a new generation starts without old or caller-forged markers. |
