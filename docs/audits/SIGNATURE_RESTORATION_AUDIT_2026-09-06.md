...
**INACTIVE.** `base44/workflows/Dispatch Scheduled Signature Reminders.jsonc` is the sole repository authority for the exact 15-minute schedule; the duplicate legacy function automation config is intentionally absent. The issuer, validator, submit broker, reminder scheduler, and reminder dispatcher have source-level `false` release gates that return a no-store 503 before request parsing, environment reads, SDK construction, service-role access, uploads, signed-URL creation, or email.
