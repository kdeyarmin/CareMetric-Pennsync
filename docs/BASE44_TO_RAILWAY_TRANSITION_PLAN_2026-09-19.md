...
| Porting the remaining 100 handlers | **Two thirds of it is not waiting on the record store.** D20 splits the `records_schema` bucket by what each module reads against the dispositions of the entities it reads: **34** read an entity dispositioned `retire`, `hub` or `preserved_paused`, so no table for it will exist here (19 touch `UserActivity`); **34** read `User`, which carries forced RLS and no policy because D14 left how it may be read undecided; only **25** are actually waiting on the store. The rest: 4 on the file layer, 1 on another port, 1 on a third-party key, 1 on `Core.SendEmail`. Eleven are written. The approach for the 25 is settled — D19's contract pattern with `listPolicyLibrary` as the worked example — so what is left in front of the other 68 is two decisions, not work. `User` is the larger: 50 of the queue touch it and 34 are held by nothing else |
