...
| Porting the remaining 100 handlers | **Not review, and no longer an open approach either.** `check:transition-disposition` classifies each `port` function by what actually blocks it: **93** read or write entity rows, **4** read or write an uploaded file and wait on the file layer, **1** calls another Base44 function, **1** needs a third-party key belonging to the runtime, and **1** (`sendWelcomeEmail`) needs `Core.SendEmail`, which the runtime does not yet broker. The 93 are no longer blocked on an unanswered question: D19 settles what a reviewed per-capability contract is and `listPolicyLibrary` is the worked example, so they are repeatable work rather than an open design. Eleven are written. Twelve left the records bucket when the modules were read and 33 arrived when the dispositions were — the category is measured from the module, never inferred |
