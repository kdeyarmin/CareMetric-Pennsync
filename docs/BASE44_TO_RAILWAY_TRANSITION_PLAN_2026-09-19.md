...
| Porting the remaining 100 handlers | **Twenty of them can be written today, and nothing in the queue is waiting on a decision any more.** D20 and D21 split the old `records_schema` bucket by what each module reads, and D23, D24 and D25 then answered all three questions that split produced. What is left is build order, not judgement: **20** wait on nothing but the work itself — D19's contract pattern, with `listPolicyLibrary` as the worked example; **43** read a roster, which D23 says is served from `pennsync_private.membership` joined to `identity_map`, so they wait on that RPC existing; **23** authorize on care-team membership, which D24 gives to `pennsync_private.assignment`, so they wait on a caller helper the policies can ask *and* a reviewed backfill out of `assigned_nurses` — both, and neither optional, because moving authority without carrying the rows across means every clinician loses their own patients on cutover; **7** read a table from a domain that is actually going away; 4 wait on the file layer, and one each on another port, a third-party key and `Core.SendEmail`. Eleven are written. The two prerequisites in front of the 66 are each a piece of source work with a test, not a meeting |
...
| --- | ---: | --- |
| `entity_authorization` | 43 | The roster RPC D23 settles: `pennsync_private.membership` joined to `identity_map`. These read `User`, which D14 left with forced RLS and no policy, and what they actually want — `agency_name`, `account_type` — is precisely what D13 refused to take from a subject's own row |
| `patient_access_model` | 23 | Both halves of D24: a caller helper the record policies can ask, granted to the record owner alone exactly as `caller_tenant_role` is, **and** a reviewed backfill from `Patient.assigned_nurses` into `pennsync_private.assignment`. Porting one of these on the helper alone means every clinician loses access to their own patients at cutover |
| `records_schema` | 20 | Nothing that is not committed. The store migration and the broker family exist; each of these needs its own reviewed contract on D19's pattern. This is the tier to start |
...
| `files` | 4 | The file layer. Porting one verbatim carries Base44's storage host into the service |
