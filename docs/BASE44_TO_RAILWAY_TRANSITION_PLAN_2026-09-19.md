...
| Porting the remaining 100 handlers | **Ten of them can be written today.** D20 and D21 split the `records_schema` bucket by what each module reads: **34** read an entity dispositioned `retire`, `hub` or `preserved_paused`, so no table for it exists here (19 touch `UserActivity`); **34** read `User`, and what they want is a roster the authority store already models and that cannot be edited by its subject (D20); **15** authorize on care-team membership, and which of the three representations of it governs has never been decided (D21) — that one answer has to serve all fifteen or the system contradicts itself about who may open a chart; **10** are actually waiting on the store; 4 on the file layer, and one each on another port, a third-party key and `Core.SendEmail`. Eleven are written. The approach for the 10 is settled — D19's contract pattern, with `listPolicyLibrary` as the worked example. What stands in front of the other 83 is three decisions, not work |
