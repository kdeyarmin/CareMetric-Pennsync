...
| D6 | Identity migration | Re-enrollment through Supabase Auth invitations with an operator-verified identity map; no password or session copy. **Extended by D99 (2026-09-25, applied):** `identity_map` can also hold a person who never held a Base44 account, as `locally_verified` with a minted id whose space is kept disjoint from Base44's by CHECK. None of the method changes for them, and the path is refused unless `PENNSYNC_ENROLL_NEW_STAFF` reads exactly `enabled-v1` — which is this decision's own question, the owner's, and has not been answered | Base44 does not export credentials; the migrating population is ten accounts |
...
says 99 functions are carried as `port`; it does not say any of them can be
...
**Re-measured 2026-09-25**, and the whole table had gone stale together — every
row, plus the count above it, which read 111. Run the tool rather than quoting
this; its `--summary` prints the counts as one line, which is the form
`AGENTS.md` is pinned to and the only one that cannot drift:

```
port queue: entity_authorization=7 files=12 external_secret=2 none=78
```

Note what that line does NOT print: a bucket that is empty. Five of the ten
blockers below have no members at all, and a reader who took the line for the
whole taxonomy would not know they exist. They are kept in the table because
what emptied each one is the useful part.

| Blocker | Functions | What has to exist first |
| --- | ---: | --- |
| `entity_authorization` | 7 | All seven UPDATE a profile, which D23 deliberately leaves open: `autoApproveInvitedUser`, `autoEndDutyDay`, `enforceStaffRoleIntegrity`, `offboardUser`, `setNurseDutyStatus`, `userManagement`, `userManagementV2`. The `MedicareGuideline` pair that used to sit here is gone — D83 retired both, since a `global` reference table is written by migration and never at runtime — and `offboardUser` arrived from `entity_not_carried` when that bucket emptied. So the bucket is smaller and also simpler: it is now one question, not two |
...
| `entity_not_carried` | 0 | Emptied by D84, which settled each uncarried leg by name in the manifest's `uncarried_legs` block rather than by re-dispositioning the capability. Each entry names the entities, what serves them instead and a reason, and the tool re-checks it against the tree — so a leg repointed, ported or retired fails the census until its entry goes with it |
...
access as waiting on the store. That bucket is now empty twice over: D75 took it
to zero on a correction, D84 deliberately put three capabilities back, and D89,
D90 and D91 wrote all three. The roster RPC and the assignment helper that stood
in front of two thirds of the queue both shipped.

So what remains is 21 capabilities across three buckets, and **none of them is
waiting on a schema**. Seven wait on a decision about the profile-write path
(D23 left it open on purpose), twelve wait on the file COPY — which is data work
against a live app and bucket, not code — and two wait on a brokered
transcription operation that has to be designed rather than configured, because
the payload is patient audio. The other 78 are written.
