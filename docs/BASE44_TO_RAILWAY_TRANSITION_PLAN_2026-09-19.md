...
| Porting the remaining 100 handlers | **Forty of them can be written today, and one prerequisite stands in front of most of the rest.** D20 and D21 split the old `records_schema` bucket by what each module reads; D23, D24 and D25 answered all three questions that split produced, and D23 and D25 are now built. What is left is build order, not judgement: **40** wait on nothing but the work itself — D19's contract pattern, with `listPolicyLibrary` as the worked example and `listAgencyRoster` as the second; **36** authorize on care-team membership, which D24 gives to `pennsync_private.assignment`, so they wait on a caller helper the policies can ask *and* a reviewed backfill out of `assigned_nurses` — both, and neither optional, because moving authority without carrying the rows across means every clinician loses their own patients on cutover; **10** need something a read policy cannot give them (8 write a profile, which D23 leaves open; 2 write a `global` reference table and need a platform ingestion path); **7** read a table from a domain that is actually going away; 4 wait on the file layer, and one each on another port, a third-party key and `Core.SendEmail`. Eleven are written. D24's two halves are the last shared prerequisite, and both are source work with a test rather than a meeting |
...
| `patient_access_model` | 36 | Both halves of D24: a caller helper the record policies can ask, granted to the record owner alone exactly as `caller_tenant_role` is, **and** a reviewed backfill from `Patient.assigned_nurses` into `pennsync_private.assignment`. Porting one of these on the helper alone means every clinician loses access to their own patients at cutover |
| `records_schema` | 40 | Nothing that is not committed. The store migration and the broker family exist; each of these needs its own reviewed contract on D19's pattern. This is the tier to start |
