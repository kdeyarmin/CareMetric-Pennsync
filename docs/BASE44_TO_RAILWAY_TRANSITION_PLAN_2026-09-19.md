...
| Phase 2 — port queue | The same gate reports what blocks each carried function rather than leaving the queue as "86 awaiting review": records_schema=80, ported_function=1, pdf_rendering=0, external_secret=1, none=4. What counts as written is read from the ported service's own registry, so a port moves a count here by existing. A test pins the distribution, so writing a port moves a count here and the prose with it |
...
| Creating the record store in a real deployment | The schema and its policies are generated and proven to apply, but no migration creates them anywhere; that needs the production Supabase project, which needs cost approval and operator credentials |
