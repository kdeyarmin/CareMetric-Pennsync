...
| Migrating the existing call sites to the ported handlers | The caller itself is built: `createStagingAuthorityClient` now exposes `callFunction`, pinned to the ported API's origin and fenced by the same lease as `rpc`, and the staging adapter routes the ten ported names to it when `VITE_PENNSYNC_API_URL` is set. It lives on the authority client because the access token never leaves that closure — handing it to a second client would undo the containment. What remains is per call site, and it is a review rather than a swap: the ported service requires an `agency_id` its Base44 original did not, so the adapter refuses a call that does not carry one rather than choosing a tenant on the caller's behalf. Also still needed: a deployed service to point it at |
