...
| Phase 2 — port queue | The same gate reports what blocks each carried function rather than leaving the queue as "86 awaiting review": records_schema=94, files=4, ported_function=1, core_integration=1, pdf_rendering=0, external_secret=1, none=10. The twelve `core_integration` ports were counted against the record store until the functions were read: every one calls a Core integration and touches no entity row, so what they waited on was the integration runtime’s brokered path — deployed, and paused — rather than a store that does not exist. All twelve left it: five by being written, two by being reclassified paused, four by being file-bound, and `generateUserGuidePDF` — the only capability that both asks a model and renders a document — by being ported once the service had a brokered integration client and a PDF library. The one counted there now arrived from the opposite direction, under D18. What counts as written is read from the ported service's own registry, so a port moves a count here by existing. A second correction followed: nine capabilities were paused at source by a module-level flag pinned `false`, so their handlers refuse before reaching anything worth porting. They are carried `preserved_paused` now, as D7 already said paused domains should be, and a gate contradicts any future `port`, `broker` or `hub` on a paused handler. A test pins the distribution and the named lists, so neither the counts nor either correction can quietly revert |
...
| Porting the remaining 101 handlers | **Not review — the data layer, for most of them.** `check:transition-disposition` classifies each `port` function by what actually blocks it: **94** read or write entity rows and need a reviewed contract against the record store for the entities they touch; **4** read or write an uploaded file, and their SSRF allowlist names Base44's own storage host, so porting one verbatim would carry a Base44 dependency into the service the exit exists to remove — they wait on the file layer; **1** calls another Base44 function and waits on that port; **1** calls a third-party API with an environment key that belongs to the runtime's brokered path; **1** (`sendWelcomeEmail`) needs `Core.SendEmail`, which the runtime does not yet broker. Twelve sat in the records bucket until the functions were read, and 33 more arrived from the opposite direction under D18 — the category has to be measured from the module, never inferred from the feature or the disposition. Ten are written: `validatePatientData`, the three documents, five handlers reaching the brokered Core integrations, and `generateUserGuidePDF`, each proved against its original by driving both |
...
| A caller for the ported handlers | **Nothing external — it is simply not built, and the plan did not say so until now.** Ten handlers are written and `src/` contains no reference to `services/pennsync-api` at all, so none of them is reachable from the app. On the Base44 path `src/functions/*` call the hosted backend; on the independent path `createIndependentStagingAdapter`'s `invoke` is an explicit allowlist that fails closed with `STAGING_OPERATION_UNAVAILABLE` for every name it does not handle, and no ported handler is among them. The service also requires an `agency_id` its Base44 original did not, so each call site is a reviewed change rather than a swap — which is why the handlers were written first, and why leaving the caller unbuilt was a choice rather than an omission. It stops being a defensible one now that the written set is ten |
