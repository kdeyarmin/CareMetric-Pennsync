...
The adapter exposes only current identity, membership/context reads and the names-only roster operation. Unsupported calls fail closed and do not fall back to Base44. The staging shell skips production navigation tracking, visual editing and remote app-settings discovery. The branded sign-in screen identifies the limited staging scope and hides Base44 signup/standard-sign-in handoffs. Legacy public capability routes, including OAuth consent, perform the existing authority teardown then show a staging-only unavailable state without mounting their provider transports; their query/fragment handles are removed. Public privacy-policy routes remain available, and production capability behavior is unchanged.
...
Terminal authority expiry, reconnect, extended background return, and entry to a public route also close local access immediately and revoke known native sessions before offering controlled reload or allowing public navigation. Initial authority boot and the first agency selection keep their valid session. Concurrent terminal transitions share one cleanup request; a cleanup failure leaves access closed with a retry action and retains the known credential in the live client. The staging reload action repeats cleanup before navigation. Page exit, browser close, a crash, or process termination can interrupt requests; the page-exit attempt is best effort only, and cannot guarantee server revocation. A retained back-forward-cache document retries cleanup before its controlled reload.
