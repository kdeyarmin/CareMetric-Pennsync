...
test('the port queue is work that cannot start yet, and says why', () => {
  ...
  // 55 → 51 → 50 → 49 → 48, and 11 → 39 written.
  ...
  assert.deepEqual(counts, { entity_not_carried: 7, entity_authorization: 10, patient_access_model: 0,
    records_schema: 48, files: 4, ported_function: 1, core_integration: 1, pdf_rendering: 0,
    external_secret: 1, none: 39 });
  ...
  // Forty-eight. That is how many of the hundred can be written today, and the
  ...
  assert.equal(report.port_blockers.records_schema.length, 48);
  // The twenty-eight that left it are the ported capabilities that touch clinical rows
