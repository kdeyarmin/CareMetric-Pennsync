...
test('the port queue is work that cannot start yet, and says why', () => {
  ...
  // 76 → 74 → 72 → 70 → 69, and 11 → 18 written.
  ...
  assert.deepEqual(counts, { entity_not_carried: 7, entity_authorization: 10, patient_access_model: 0,
    records_schema: 69, files: 4, ported_function: 1, core_integration: 1, pdf_rendering: 0,
    external_secret: 1, none: 18 });
  ...
  // Sixty-nine. That is how many of the hundred can be written today, and the
  ...
  assert.equal(report.port_blockers.records_schema.length, 69);
  // The seven that left it are the ported capabilities that touch clinical
  // rows — D26's patient pair, then the visit and document pairs on the same
  // machinery, then the first WRITE — so they are also the proof that the D19
  // pattern carries PHI and not only configuration. The document pair
  // additionally shows that `files` was never the blocker there: no purpose
  // discloses a locator.
  ...
  assert.deepEqual(report.port_blockers.none,
    ['analyzeReferral', 'analyzeReferralIntake', 'analyzeReferralPriority',
      'createAuthorizedPatient', 'generateBagTechniquePDF',
