...
test('the port queue is work that cannot start yet, and says why', () => {
  ...
  assert.deepEqual(counts, { entity_not_carried: 7, entity_authorization: 10, patient_access_model: 0,
    records_schema: 49, files: 4, ported_function: 1, core_integration: 1, pdf_rendering: 0,
    external_secret: 1, none: 38 });
  ...
  // Forty-nine. That is how many of the hundred can be written today, and the
  ...
  assert.equal(report.port_blockers.records_schema.length, 49);
  // The twenty-seven that left it are the ported capabilities that touch clinical rows
