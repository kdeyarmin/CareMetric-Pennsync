...
test('the port queue is work that cannot start yet, and says why', () => {
  ...
  assert.deepEqual(counts, { entity_not_carried: 7, entity_authorization: 8, patient_access_model: 0,
    records_schema: 5, files: 12, ported_function: 1, core_integration: 2, pdf_rendering: 0,
    external_secret: 2, none: 68 });
  ...
  // Five. That is how many of the hundred can be written today, and the
  ...
  assert.equal(report.port_blockers.records_schema.length, 5);
  // The thirty-five that left it are the ported capabilities that touch clinical rows
