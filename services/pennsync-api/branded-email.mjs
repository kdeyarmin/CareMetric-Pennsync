// The branded PennSync email, carried from the Base44 originals.
//
// D97. This is a COPY of the generated `brandedEmail` shared helper as it is
// emitted into `base44/functions/sendAccountReadyEmail/entry.ts`, not a
// reimplementation of it. D81 set that rule for the patient handout's twenty
// templates and the reason is the same here: the output is a message a person
// reads, and a renderer retyped from a reading drifts in ways no assertion
// catches. `branded-email.test.mjs` re-extracts the block from the original and
// renders both over the same fixtures, so a divergence fails the build.
//
// Three deliberate differences from the block, each of which a test pins:
//
// 1. `renderBrandedEmail` and `escapeEmailHtml` are exported; everything else
//    stays module-private exactly as the helper's own scope has it.
// 2. `BRAND_EMAIL.logo` is the original's absolute URL, and it stays one. It is
//    a Base44 storage address, which is the coupling the surface ratchet
//    forbids in `src/` — this is a backend service, the ratchet does not reach
//    it, and substituting a different host would change what a recipient sees.
//    Replacing it is a brand decision with an owner, not a port's to take.
// 3. Nothing else. The helper reads no environment, no clock and no request, so
//    there is nothing else to adapt.
//
// The escaping is the security property worth naming: every interpolated value
// goes through `escapeEmailHtml`, and a button's href additionally through
// `safeEmailHref`, which admits only absolute http(s) and mailto. A caller
// therefore cannot inject markup or a scheme through a name, an address or a
// link, which matters more here than in the original because the fields these
// senders interpolate are operator-supplied.
