// PennSync user manual, ported from base44/functions/generateUserManual/entry.ts.
//
// The layout is the original's, call for call; `pennsyncApiDocumentParity` runs
// both against one recording surface and compares. Nothing is changed: this
// original fetched no logo and read no clock, so it ports verbatim.
//
// The manual text is user-facing documentation. Editing a line here changes
// what a reader is told the product does.

export const USER_MANUAL_FILENAME = 'PennSync_User_Manual.pdf';

export function buildUserManual(doc) {
