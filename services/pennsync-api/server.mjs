...
const server = createServer(async (incoming, outgoing) => {
  try {
    ...
    const req = new Request(new URL(incoming.url || '/', 'http://pennsync-api.invalid'), {
    ...
    outgoing.end('{"success":false,"error":"PENNSYNC_API_UNAVAILABLE"}');
  }
});

// These bound incoming request and header consumption only.
server.requestTimeout = 15000;
server.headersTimeout = 10000;
server.maxConnections = 100;
server.listen(port, '0.0.0.0', () => process.stdout.write(JSON.stringify({
  event: 'pennsync_api_started',
  revision: config.revision,
  released: config.released,
  operations: publicReadiness(config).operations,
}) + '\n'));
