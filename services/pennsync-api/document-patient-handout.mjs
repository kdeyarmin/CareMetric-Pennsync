...
//
// **This is a PARTIAL port, and which half is which is the whole point.**
// The original has two actions. `action === 'email'` reaches
// `Core.SendEmail`; the document action (the default) reaches no integration
// at all and answers with a rendered PDF. The port serves the document action
// and reports the email one paused, which is the shape D42, D49, D50, D52,
// D54 and D73 already ship.
//
// D79 recorded why that distinction was invisible: the port queue's
// `core_integration` rule is `/\.\s*integrations\s*\./`, it answers on the
// SHAPE of the call, and nothing asked whether the call sits on a path the
// module itself already refuses. This one does -- the original's own
// `outboundDeliveryGate` returns before any work when the release flag is
// unset -- so the capability was counted as blocked on an owner decision when
// only half of it was.
//
// The layout is the original's, call for call; `pennsyncApiDocumentParity`
// runs both against one recording surface and compares. Three things are
// deliberately not carried:
//
// - **The logo is supplied, never fetched.** The original fetched a PNG from
//   Base44's own storage bucket on every request. `documents.mjs` says why
//   that cannot come with us, and the builder takes the branch the original
//   already took when that fetch failed.
// - **The date is supplied.** The original called `new Date()` inside the
//   render, so the same request produced a different document either side of
//   midnight and its parity could not be tested.
// - **`diagnostics` is not in the answer.** The original reports the render
//   stage and a section count back to the caller. They are the bookkeeping of
//   its own inline try/catch staging, which this builder does not have, and
//   `src/pages/PatientEducationHub.jsx` reads neither -- the `diagnostics`
//   object in that page is its own, built client-side.
// - **The fallback-PDF-on-error path is absent, not paused.** The original
//   answers `success: true` with a generic "contact your nurse" document when
//   its own render throws, which turns a render defect into a handout the
//   patient cannot use and the nurse is never told about. It carried the
//   diagnostics into `SystemLog`, a D25 retired table. Here a render failure
//   is a refusal the caller can retry, and the trail is the D25 successor.
//
// The clinical content below is reproduced verbatim from the original: these
// are the words a patient reads about their own condition, and changing a line
// here changes medical education material.


// Interactive elements for handouts
export const INTERACTIVE_RESOURCES = {
  'chf': [
...
export const TRACKING_CHECKLISTS = {
...
// Strip non-ASCII so jsPDF's core fonts render cleanly. Verbatim from the original.
const clean = (s) => String(s || '').normalize('NFKD').replace(/[̀-ͯ]/g, '').replace(/[^\x00-\x7F]/g, '');

/** Matches the original's `new Date().toLocaleDateString('en-US', …)` on the patient card. */
export const handoutDate = (date = new Date()) =>
  date.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });

/** The filename the original answered with. */
export const handoutFilename = (condition) => `${condition}_handout.pdf`;

/**
 * Resolves the caller's `styleOptions` exactly as the original does, including
 * every default. Separated from the builder so the handler can validate a
 * request without rendering, and so parity is provable on the defaults alone.
 */
...
/**
 * Renders the handout, call for call as the original does.
 *
 * `preparedOn` is the date on the patient card and `logoDataUrl` the banner
 * image; both are supplied rather than reached for, for the reasons at the top
 * of this file. The builder refuses to invent the date, so a caller that
 * forgets one fails loudly instead of rendering an undated clinical document.
 */
export function buildPatientHandout(doc, {
  condition, patientName, selectedSections, customNotes, style,
} = {}, { logoDataUrl = null, preparedOn } = {}) {
  if (typeof preparedOn !== 'string' || !preparedOn) throw new TypeError('preparedOn is required');
  const template = HANDOUT_TEMPLATES[condition];
  if (!template) throw new TypeError(`Invalid condition: ${condition}`);

  doc.setProperties({
    title: template.title,
    subject: 'Patient Education Material',
    author: style.agencyName,
    keywords: 'patient education, healthcare, ' + condition,
    creator: 'PennSync Documentation System',
    language: 'en-US',
  });
  ...
  const config = LAYOUT_CONFIG[style.layout] || LAYOUT_CONFIG.standard;
  const margin = config.margin;
  const contentWidth = pageWidth - 2 * margin;

  const COLORS = {
    ...COLOR_SCHEMES[style.colorScheme],
  ...
  const paintBanner = () => {
    ...
    // The original added the image here when its fetch succeeded, and took the
    // lettered branch when it did not.
    if (logoDataUrl) {
  ...
  doc.text(preparedOn, pageWidth - margin - 6, yPos + 12, { align: 'right' });
  yPos += 22;
  ...
  const checklist = TRACKING_CHECKLISTS[condition];
  ...
  const resources = INTERACTIVE_RESOURCES[condition];
