...
The real gateway tests use the modern local secret key only in this Node fixture runner and the publishable key for anonymous denials. Keys remain in memory. The suite checks reserve/complete/replay, exact file ownership and cleanup RPC behavior without invoking a provider or uploading a file. Every business POST runs once. Always run the authority harness's scoped `stop` afterward, including on failure. No hosted project is linked or changed. Actual Docker-backed success must be recorded separately; passing the eight native tests is not a substitute.

Before any business request, the suite checks all five database function names, argument names/types, trusted owners and service-only execution grants. It then waits for each exact function signature to resolve through the API's schema cache. Each probe is an anonymous GET with valid synthetic argument types, the publishable key and no Bearer credential. It must return HTTP 401 with SQLSTATE 42501 naming that function. The verified anonymous execution denial prevents function bodies from running; GET also selects a read-only transaction. Only HTTP 404/PGRST202 cache misses are retried, under one ten-second deadline with 16 KiB response bounds. Network, authentication, malformed or unexpected responses fail immediately with fixed error codes. All three integration tables must still be empty after readiness, before the one-shot business tests begin.

Checking only `reserve` was insufficient: a cache captured after migration 001 can resolve it while the cleanup function introduced by 003 remains absent. The 18 pure readiness tests model that partial cache and prove the later function gates progress, permanent failures stop polling, credentials remain confined to the fixed literal loopback endpoint, and stalled fetch/body cancellation cannot defeat the deadline. These tests run in both the normal external-integration suite and the real API workflow before stack startup.

This gate deliberately uses cached anonymous GET resolution rather than OpenAPI or OPTIONS as readiness evidence. In pinned PostgREST 14.14, default OpenAPI [reads live function metadata](https://github.com/PostgREST/postgrest/blob/v14.14/src/PostgREST/MainTx.hs), so it can describe functions absent from the cache. [Request parsing](https://github.com/PostgREST/postgrest/blob/v14.14/src/PostgREST/ApiRequest.hs) only treats RPC GET/HEAD argument values as invocation arguments, excluding OPTIONS. [Call planning](https://github.com/PostgREST/postgrest/blob/v14.14/src/PostgREST/Plan.hs) resolves GET argument names against the cache and uses a read-only transaction. This is a disposable-local startup check, not production migration or hosted readiness evidence.
