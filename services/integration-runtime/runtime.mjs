...
import { IntegrationError, fail, ID, OPERATIONS, hash, readJson, seal, stable, unseal } from './safety.mjs';
...
export function loadConfig(env = process.env) {
  ...
  if (operations.some(operation => !OPERATIONS.includes(operation))) throw new Error('INVALID_OPERATION_CONFIGURATION');
  ...
  return {
    ...
    released: env.INTEGRATIONS_RELEASE === 'enabled-v1',
    dailyLimit: 100,
    supabaseKey: env.SUPABASE_SERVICE_ROLE_KEY || '',
    anthropicKey: env.ANTHROPIC_API_KEY || '',
    model: env.INTEGRATIONS_AI_MODEL || 'claude-sonnet-4-6',
...
export function publicReadiness(config) {
  return { ready: config.configured && config.released && config.operations.length > 0,
    released: config.released, configured: config.configured, operations: config.operations,
...
export async function authorize(config, req, agencyId, fetcher = fetch) {
  ...
  // The existing tenant broker is deliberately retained during migration.
  // This call remains a Base44 execution dependency, not a zero-credit claim.
  ...
  if (!c || !ID.test(c.user_id || '') || c.agency_id !== agencyId || c.agency?.id !== agencyId
  ...
  const owner = c.is_platform_owner === true && c.tenant_role === 'platform_owner'
    && c.membership_id === null && c.membership_version === null;
...
export function createStore(config, fetcher = fetch) {
  ...
  return {
    reserve: body => rpc('cm_integration_reserve', body),
    finish: body => rpc('cm_integration_finish', body),
    fileGet: body => rpc('cm_integration_file_get', body),
    fileRecord: body => rpc('cm_integration_file_record', body),
  };
}
export async function performDurable({ config, req, agencyId, operation, params, requestId, provider, store, authority }) {
  ...
  if (typeof reservation.id !== 'string' || !/^[0-9a-f-]{36}$/.test(reservation.id)) fail(503, 'INVALID_RESERVATION');
  let started = false;
  try {
    ...
    if (reservation.outcome === 'completed') return unseal(config.encryptionKey, `${config.appId}:${before.subject}:${reservation.id}`, reservation.result);
    started = true;
    const result = await provider(operation, params, { subject: before.subject, jobId: reservation.id });
