...
export const importPatients = Object.freeze([
  Object.freeze({ id: 'f10000000000000000000001', agency_id: 'agency-a', display_name: 'Synthetic Archive Patient A', synthetic: true, version: 1, status: 'active' }),
  Object.freeze({ id: 'f20000000000000000000002', agency_id: 'agency-b', display_name: 'Synthetic Archive Patient B', synthetic: true, version: 1, status: 'active' }),
]);
...
export async function seedImportReceipt(db) {
  ...
  assert.equal(importPatients.every(patient => /^[a-f0-9]{24}$/.test(patient.id)), true);
  ...
  try {
    ...
    await db.query(`insert into pennsync_private.archive_patient_import_receipt
      (app_id,plan_sha256,owner_sha256,projection_sha256,patient_count,patient_ids,state) values($1,$2,$3,$4,$5,$6::text[],'applied')`,
    [APP, planHash, ownerHash, projectionHash, importPatients.length, importPatients.map(patient => patient.id).sort()]);
...
export async function proveImportReceipt(db, { planHash, receipt }) {
  ...
  assert.deepEqual(restored.patient_ids, patients.map(patient => patient.id).sort(), 'Restored per-patient tombstone identities match the actual business rows');
  ...
  return { import_receipt_fields_preserved: true, import_patient_projection_hash: true, import_receipt_patient_ids_preserved: true,
