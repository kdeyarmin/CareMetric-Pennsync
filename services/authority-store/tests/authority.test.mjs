...
scenario('no caller-supplied owner, role or agency claims create grants',async()=>{
  ...
  await denied(()=>rpc('context',['694ec16e72e01b60d22f7cbf','agency-a']),'PENNSYNC_STAGING_ONLY');
});
...
scenario('database identity provenance, duplicate memberships and cross-agency assignment FKs cannot be bypassed',async()=>{
  ...
  await denied(()=>db.exec("insert into pennsync_private.assignment values('6a9881683dc68a0bd54f1ef7','agency-b','patient-b1','membership-2','active',1,'10000000-0000-4000-8000-000000000001',clock_timestamp())"));
...
scenario('browser table CRUD, internal helper execution and anonymous wrappers are denied',async()=>{
  for(const name of ['identity_map','agency','membership','patient','assignment','mutation_receipt','archive_patient_import_receipt']) {
...
scenario('public wrappers are invoker-only; private grants and RLS are complete',async()=>{
  ...
  assert.deepEqual(rows.rows.map(x=>x.proname).sort(),['context','memberships','patients','patient','assignment','revoke_membership','s4_create','s4_read','s3_create','s3_confirm','s3_read'].map(x=>`pennsync_staging_${x}`).sort());
  ...
  assert.deepEqual(tables.rows.map(x=>x.relname).sort(),['identity_map','agency','membership','patient','assignment','mutation_receipt','archive_patient_import_receipt','s4_visit','s4_note_history','s4_note_conversion','s4_compliance_audit','s4_create_receipt','s3_referral','s3_receipt'].sort());
