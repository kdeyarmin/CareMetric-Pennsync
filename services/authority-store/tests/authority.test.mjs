...
scenario('browser table CRUD, internal helper execution and anonymous wrappers are denied',async()=>{
  for(const name of ['identity_map','agency','membership','patient','assignment','mutation_receipt','archive_patient_import_receipt','visit_disclosure_audit']) {
...
scenario('public wrappers are invoker-only; private grants and RLS are complete',async()=>{
  ...
  assert.deepEqual(rows.rows.map(x=>x.proname).sort(),['context','memberships','patients','patient','assignment','revoke_membership','s4_create','s4_read','s3_create','s3_confirm','s3_read','visit_documentation'].map(x=>`pennsync_staging_${x}`).sort());
  ...
  assert.deepEqual(tables.rows.map(x=>x.relname).sort(),['identity_map','agency','membership','patient','assignment','mutation_receipt','archive_patient_import_receipt','visit_disclosure_audit','s4_visit','s4_note_history','s4_note_conversion','s4_compliance_audit','s4_create_receipt','s3_referral','s3_receipt'].sort());
