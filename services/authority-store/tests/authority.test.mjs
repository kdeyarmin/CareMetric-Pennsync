...
scenario('browser table CRUD, internal helper execution and anonymous wrappers are denied',async()=>{
  for(const name of ['identity_map','agency','membership','patient','assignment','mutation_receipt']) {
...
scenario('public wrappers are invoker-only; private grants and RLS are complete',async()=>{
  ...
  assert.deepEqual(tables.rows.map(x=>x.relname).sort(),['identity_map','agency','membership','patient','assignment','mutation_receipt','s4_visit','s4_note_history','s4_note_conversion','s4_compliance_audit','s4_create_receipt','s3_referral','s3_receipt'].sort());
