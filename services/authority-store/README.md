...
The migration and local SQL tests passed against PostgreSQL 17.10 and PGlite. Supabase CLI 2.109.1 security advisors also returned no warning/error findings against an empty disposable local database with this schema (`--db-url` with local `sslmode=disable`, `--type security --level warn --fail-on error`). The initial CLI connection without explicit local SSL mode failed and was not counted as an advisor pass. The base authority Docker-backed CI suite passed actual local Auth password login, signed sessions and HTTP/PostgREST acceptance. The additional S4 HTTP checks need their own successful Docker-backed run; see `tests/http-acceptance.md`. No hosted enrollment/migration, customer patient-data migration, production cutover or full clinical workflow completion is claimed here.
