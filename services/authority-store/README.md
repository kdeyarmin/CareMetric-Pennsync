...
| `20260920110000_claim_new_chart.sql` | Not a contract and not a record-store object: the cross-store bridge (D28). `pennsync_claim_new_chart(agency)` mints a chart identity and takes the caller's own care-team seat, so that a creator can open what they created — creating a chart and being on its care team are writes to two owners. It takes an agency and nothing else, because a caller who could name the id would name a chart that already exists. It is here rather than in the authority directory by dependency order: it asks `caller_tenant_role` |
