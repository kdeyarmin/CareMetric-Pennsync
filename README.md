...
- `VITE_CENTRAL_HELP_ENABLED` — optional first-party Support Hub control for the production CareMetric Base44 app. The production build is enabled by its immutable app ID when the variable is omitted; an explicit value other than the exact string `true` disables it. Preview/development and every other app ID remain off. `/Help` remains available as the local fallback.
- `VITE_DEPLOY_ENV` — allowlisted Support Hub context label: `production`, `staging`, or `development`.
