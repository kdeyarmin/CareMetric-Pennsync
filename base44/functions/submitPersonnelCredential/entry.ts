...
Deno.serve(async (req) => {
  try {
    ...
    if (Object.entries(fields).some(([key, value]) => typeof value !== 'string' || value.length > (key === 'notes' ? 4000 : 2000))
      || !String(fields.title).trim() || !exactDay(fields.expiration_date)
      || (fields.issued_date && (!exactDay(fields.issued_date) || fields.issued_date > fields.expiration_date))) {
      return Response.json({ error: 'Credential fields must be text with valid issue/expiration dates in the correct order.' }, { status: 400 });
    }
    if (fields.uploaded_file_url) {
      try {
        const file = new URL(fields.uploaded_file_url);
        if (file.protocol !== 'https:' || file.username || file.password) throw new Error('Unsupported file URL');
      } catch {
        return Response.json({ error: 'Use a valid HTTPS credential-file link.' }, { status: 400 });
      }
    }
