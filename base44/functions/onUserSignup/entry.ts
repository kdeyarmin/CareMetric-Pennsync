...
Deno.serve(async (req) => {
  try {
    ...
    A sign-up attempt was BLOCKED on PennSync by CareMetric because the account was not invited.

    PennSync by CareMetric is invite-only — there is no public sign-up. This account has NOT
    ...
    Best regards,
    PennSync by CareMetric Security
    `.trim();
    ...
    const emailPromises = admins.map(admin =>
      base44.asServiceRole.integrations.Core.SendEmail({
        to: admin.email,
        subject: `🚫 Blocked Uninvited Sign-up - PennSync by CareMetric`,
        from_name: 'PennSync by CareMetric Security',
