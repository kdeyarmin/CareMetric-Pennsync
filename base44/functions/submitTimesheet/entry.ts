...
Deno.serve(async (req) => {
  try {
    ...
    let profile = null;
    try {
      const profiles = await base44.asServiceRole.entities.EmployeePayrollProfile.filter({ employee_email: user.email }, undefined, 2);
      if (!Array.isArray(profiles) || profiles.some(row => !row || row.employee_email !== user.email)) throw new Error('Payroll profile lookup is incomplete');
      if (profiles.length > 1) return Response.json({ error: 'Multiple payroll profiles require administrator reconciliation before saving.' }, { status: 409 });
    ...
    if (body.entry_mode === 'daily') {
      const entries = body.daily_entries;
      if (!Array.isArray(entries) || entries.length > 14
        || new Set(entries.map(entry => entry?.date)).size !== entries.length
        || entries.some(entry => !entry || typeof entry !== 'object' || !parseISODate(entry.date)
          || entry.date < pay_period_start || entry.date > pay_period_end
          || DAILY_SUM_FIELDS.some(field => !validNumber(entry[field]))
          || VISIT_TYPE_KEYS.some(key => !validNumber(entry.visit_counts?.[key])))) {
        return Response.json({ error: 'Daily entries must use unique valid dates within this pay period and non-negative numbers.' }, { status: 400 });
      }
    }
    ...
    if (earns_points) {
      ...
      try {
        ...
        if (agency) {
          ...
          if (!Array.isArray(configs) || configs.length >= 10 || configs.some(row => row?.agency_name !== agency)) throw new Error('Point schedule lookup is incomplete');
        }
        if (!configs?.length) {
          ...
          const newest = await base44.asServiceRole.entities.VisitPointConfig
            .list('-updated_date', 5);
          if (!Array.isArray(newest)) throw new Error('Point schedule lookup failed');
        ...
        const activeConfigs = (configs || []).filter(row => row && row.active !== false);
        if (activeConfigs.length > 1 || (!activeConfigs.length && Object.values(visit_counts).some(count => count > 0))) {
          return Response.json({ error: 'A single active visit-point schedule is required before saving visit points.' }, { status: 409 });
        }
        const cfg = activeConfigs[0] || {};
        computedPoints = computeVisitPoints(visit_counts, cfg);
      } catch (_cfgError) {
        return Response.json({ error: 'Visit-point values could not be verified. Nothing was saved.' }, { status: 503 });
      }
    }
    ...
    try {
      ...
      if (!Array.isArray(approved) || approved.length >= 5000 || approved.some(row => !row || row.employee_email !== user.email || row.status !== 'approved')) throw new Error('PTO lookup is incomplete');
      auto_pto_hours = computePtoHours(approved, pay_period_start, pay_period_end);
    } catch (_ptoError) {
      return Response.json({ error: 'Approved PTO could not be verified. Nothing was saved; retry when the service is available.' }, { status: 503 });
    }
    ...
    const siblingsForPeriod = async (excludeId) => {
      ...
      if (!Array.isArray(matches) || matches.length >= 5000 || matches.some(row => !row || row.employee_email !== user.email || row.service_type !== service_type || row.pay_period_start !== pay_period_start || row.pay_period_end !== pay_period_end)) throw new Error('Timesheet lookup is incomplete');
    ...
    if (timesheet_id) {
      ...
      if (siblings.length > 1) {
        return Response.json({ error: 'Multiple timesheets exist for this period. Ask an administrator to reconcile them before resubmitting.' }, { status: 409 });
      }
      ...
      if (!reusable) {
        // Do not delete records or overwrite another submitter's winning row
        // after a race. Preserve evidence and reject an ambiguous outcome.
        const dupes = await siblingsForPeriod(saved.id);
        if (dupes.length > 0) {
          return Response.json({ error: 'A simultaneous submission created duplicate timesheets. Do not resubmit; an administrator must reconcile this period.' }, { status: 409 });
        }
      }
    }
