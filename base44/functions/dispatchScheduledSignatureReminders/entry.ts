...
Deno.serve(async (req) => {
  try {
    ...
    // user). Opt-in lockdown mirroring dispatchScheduledSms — a real scheduler
    // runs unauthenticated and still passes while no secret is configured; once
    // INTERNAL_FN_SECRET is set it must present the header, and a logged-in
