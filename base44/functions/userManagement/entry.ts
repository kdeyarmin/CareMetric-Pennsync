...
async function inviteUser(base44, currentUser, params, isAdmin, callerIsSuperAdmin) {
  ...
  try {
    ...
    await base44.asServiceRole.integrations.Core.SendEmail({
      to: email,
      subject: 'Invitation to PennSync by CareMetric',
      body: `Hello ${full_name},\n\nYou've been invited to join PennSync by CareMetric.\n\nEmail: ${email}\nRole: ${role || 'user'}\n\nPlease visit ${signupUrl} to create your account.\n\n⏰ This invitation expires in 7 days (${expiresAt.toLocaleDateString()}).\n\nWelcome to PennSync by CareMetric!`,
...
async function resendInvitation(base44, currentUser, params, isAdmin) {
  ...
  await base44.asServiceRole.integrations.Core.SendEmail({
    to: invitation.email,
    subject: 'Reminder: Invitation to PennSync by CareMetric',
    body: `Hello ${invitation.full_name},\n\nThis is a reminder that you've been invited to join PennSync by CareMetric.\n\nEmail: ${invitation.email}\nRole: ${invitation.role || 'user'}\n\nPlease visit ${signupUrl} to create your account.\n\n⏰ This invitation expires in 7 days (${newExpiresAt.toLocaleDateString()}).\n\nWelcome to PennSync by CareMetric!`,
    from_name: 'PennSync by CareMetric'
  });
