...
function requireCleanBuildingOutcomeRun(preimage) {
  ...
  if (OUTCOME_RUN_TERMINAL_FIELDS.some((field) => preimage[field] != null)) {
...
function requireExactCreatedBuildingRun(created, expected) {
  ...
  if (Object.entries(intended).some(([field, value]) =>
    !sameOutcomeRunValue(actual[field], value)
      && !(value === undefined && actual[field] === null
        && (field === 'benchmark_value' || OUTCOME_RUN_TERMINAL_FIELDS.includes(field))))) {
...
// An existing Agency row owns each reporting window. Committing ownership is
// retained until its exact run is terminal; expiration alone never authorizes
// a second publication while the first status write may still complete.
const OUTCOME_WINDOW_CLAIM_LIMIT = 500;
const exactOutcomeId = value => typeof value === 'string' && value.length > 0 && value.length <= 200
  && value.trim() === value && !value.startsWith('$') && !/[\u0000-\u001f\u007f]/.test(value);
const exactOutcomeInstant = value => typeof value === 'string' && Number.isFinite(Date.parse(value))
  && new Date(value).toISOString() === value;
const exactOutcomeCas = value => value?.success === true && value.updated === 1 && value.has_more === false;

async function loadOutcomeClaimAgency(entities, agencyId) {
  const rows = await entities.Agency.filter({ id: agencyId }, undefined, 2);
  if (!Array.isArray(rows) || rows.length !== 1 || rows[0]?.id !== agencyId
    || typeof rows[0].updated_date !== 'string' || !Number.isFinite(Date.parse(rows[0].updated_date))) throw new Error('Outcome claim Agency is unavailable');
  return rows[0];
}
function outcomeClaimMap(agency) {
  const value = agency.outcome_window_claims;
  if (value == null) return {};
  if (typeof value !== 'object' || Array.isArray(value) || Object.keys(value).length > OUTCOME_WINDOW_CLAIM_LIMIT) {
    throw new Error('Outcome claim map is invalid');
  }
  for (const [key, claim] of Object.entries(value)) {
    if (!/^[a-f0-9]{64}$/.test(key) || !claim || typeof claim !== 'object' || Array.isArray(claim)
      || Object.keys(claim).sort().join('|') !== 'expires_at|phase|run_id|started_at|token'
      || !/^[a-f0-9]{8}-[a-f0-9]{4}-4[a-f0-9]{3}-[89ab][a-f0-9]{3}-[a-f0-9]{12}$/.test(claim.token)
      || !['building', 'committing'].includes(claim.phase)
      || !exactOutcomeInstant(claim.started_at) || !exactOutcomeInstant(claim.expires_at)
      || Date.parse(claim.expires_at) - Date.parse(claim.started_at) !== OUTCOME_RUN_LEASE_MS
      || (claim.phase === 'building' ? claim.run_id !== null : !exactOutcomeId(claim.run_id))) {
      throw new Error('Outcome window claim is invalid');
    }
  }
  return value;
}
async function updateOutcomeClaimMap(entities, agency, next) {
  return entities.Agency.updateMany({
    id: agency.id, status: agency.status, updated_date: agency.updated_date,
    outcome_window_claims: Object.hasOwn(agency, 'outcome_window_claims')
      ? agency.outcome_window_claims : { $exists: false },
  }, { $set: { outcome_window_claims: next } }).catch(() => null);
}
async function loadOutcomeClaimRun(entities, agencyId, windowKey, claim) {
  const rows = await entities.OutcomeComputationRun.filter({ agency_id: agencyId, id: claim.run_id }, undefined, 2);
  if (!Array.isArray(rows) || rows.length !== 1 || rows[0]?.id !== claim.run_id
    || rows[0].agency_id !== agencyId || rows[0].window_key !== windowKey
    || rows[0].attempt_id !== claim.token || rows[0].started_at !== claim.started_at
    || rows[0].lease_expires_at !== claim.expires_at || outcomeRunLeaseValidationError(rows[0])) {
    throw new Error('Outcome committing run is unavailable');
  }
  return rows[0];
}
async function reconcileCommittingOutcomeClaim(entities, agencyId, windowKey, claim, nowMs) {
  let row = await loadOutcomeClaimRun(entities, agencyId, windowKey, claim);
  if (row.status === 'building') {
    if (Date.parse(claim.expires_at) > nowMs) return false;
    try {
      await applyConditionalOutcomeRunTransition(entities.OutcomeComputationRun, outcomeRunSnapshot(row), {
        status: 'failed', failed_at: new Date(nowMs).toISOString(), failure_stage: 'expired_run_lease_reconciled',
      });
    } catch {
      // Publication may have won the same run CAS, or the failure response may
      // be lost. Only exact terminal readback permits releasing the window.
    }
    row = await loadOutcomeClaimRun(entities, agencyId, windowKey, claim);
  }
  return ['published', 'failed', 'superseded'].includes(row.status) && row.transition_version === 2;
}
async function acquireOutcomeWindowClaim(entities, agencyId, windowKey) {
  if (!exactOutcomeId(agencyId) || !/^[a-f0-9]{64}$/.test(windowKey)) throw new Error('Outcome claim scope is invalid');
  for (let attempt = 0; attempt < 6; attempt++) {
    const agency = await loadOutcomeClaimAgency(entities, agencyId);
    if (!['active', 'trial'].includes(agency.status)) throw new Error('Outcome Agency is inactive');
    const claims = outcomeClaimMap(agency);
    const current = claims[windowKey];
    const nowMs = Date.now();
    if (current) {
      if (current.phase === 'building' && Date.parse(current.expires_at) > nowMs) return null;
      if (current.phase === 'committing') {
        if (!await reconcileCommittingOutcomeClaim(entities, agencyId, windowKey, current, nowMs)) return null;
        const next = { ...claims };
        delete next[windowKey];
        if (!exactOutcomeCas(await updateOutcomeClaimMap(entities, agency, next))) continue;
        return { reconciled: true };
      }
    } else if (Object.keys(claims).length >= OUTCOME_WINDOW_CLAIM_LIMIT) {
      throw new Error('Outcome pending claim capacity exceeded');
    }
    const claim = { token: crypto.randomUUID(), phase: 'building', run_id: null,
      started_at: new Date(nowMs).toISOString(), expires_at: new Date(nowMs + OUTCOME_RUN_LEASE_MS).toISOString() };
    await updateOutcomeClaimMap(entities, agency, { ...claims, [windowKey]: claim });
    // A lost acknowledgement is recoverable only by our unique stored token.
    const verified = await loadOutcomeClaimAgency(entities, agencyId);
    if (sameOutcomeRunValue(outcomeClaimMap(verified)[windowKey], claim)) {
      return { agencyId, windowKey, claim };
    }
  }
  return null;
}
async function beginOutcomePublication(entities, ownership, runId) {
  for (let attempt = 0; attempt < 6; attempt++) {
    const agency = await loadOutcomeClaimAgency(entities, ownership.agencyId);
    const claims = outcomeClaimMap(agency);
    if (!['active', 'trial'].includes(agency.status)
      || !sameOutcomeRunValue(claims[ownership.windowKey], ownership.claim)
      || ownership.claim.phase !== 'building' || Date.parse(ownership.claim.expires_at) <= Date.now()) return false;
    const committing = { ...ownership.claim, phase: 'committing', run_id: runId };
    await updateOutcomeClaimMap(entities, agency, { ...claims, [ownership.windowKey]: committing });
    const verified = await loadOutcomeClaimAgency(entities, ownership.agencyId);
    if (!sameOutcomeRunValue(outcomeClaimMap(verified)[ownership.windowKey], committing)) continue;
    ownership.claim = committing;
    return true;
  }
  return false;
}
async function releaseOutcomeWindowClaim(entities, ownership) {
  for (let attempt = 0; attempt < 6; attempt++) {
    const agency = await loadOutcomeClaimAgency(entities, ownership.agencyId);
    const claims = outcomeClaimMap(agency);
    const current = claims[ownership.windowKey];
    if (!current || current.token !== ownership.claim.token) return true;
    if (current.phase === 'committing') {
      const row = await loadOutcomeClaimRun(entities, ownership.agencyId, ownership.windowKey, current);
      if (!['published', 'failed', 'superseded'].includes(row.status) || row.transition_version !== 2) return false;
    }
    const next = { ...claims };
    delete next[ownership.windowKey];
    if (exactOutcomeCas(await updateOutcomeClaimMap(entities, agency, next))) return true;
  }
  return false;
}
...
Deno.serve(async (req) => {
  ...
  let windowOwnership = null;
  let windowClaimClient = null;
  try {
    ...
    const agencyRows = await base44.asServiceRole.entities.Agency.filter({ id: agencyId }, undefined, 2);
    if (!Array.isArray(agencyRows) || agencyRows.length !== 1 || agencyRows[0]?.id !== agencyId) {
      return Response.json({ error: 'Agency not found or ambiguous' }, { status: 404 });
    }
    if (!['active', 'trial'].includes(agencyRows[0].status)) {
      return Response.json({ error: 'Agency is inactive' }, { status: 409 });
    }
    ...
    // Capture every cohort before re-reading any cohort. With append-only OASIS
    // rows and revisioned Patient records, equal complete collections share a
    // boundary between the two passes. A changed or incomplete source fails
    ...
    const readAssessmentCohort = async (query, maxRows) => {
      ...
      while (true) {
        ...
        if (!Array.isArray(page) || page.length > limit) throw new Error('Invalid assessment source page');
        for (const row of page) {
          ...
          if (query.assessment_date && Number.isFinite(Date.parse(row.assessment_date))
            && (Date.parse(row.assessment_date) < periodStartMs || Date.parse(row.assessment_date) >= periodEndExclusiveMs)) {
            throw new Error('Assessment source escaped reporting period');
          }
          cursor = row.id;
    ...
    const captureAssessments = async (query, maxRows) => {
      const key = JSON.stringify(canonicalOutcomeJsonValue(query));
      if (sourceCohorts.has(key)) return sourceCohorts.get(key).page;
      const page = await readAssessmentCohort(query, maxRows);
      sourceRowCount += page.rows.length;
      if (sourceRowCount > 100_000) throw new Error('Combined outcome source capacity exceeded; split the period');
      sourceCohorts.set(key, { query, maxRows, page, hash: await resultSummaryHash(page.rows) });
      return page;
    };
    ...
    const capturePatient = async (patientId) => {
    ...
    const verifySourceSnapshot = async () => {
      ...
      const fingerprints = [];
      for (const [key, cohort] of sourceCohorts) {
        const reread = await readAssessmentCohort(cohort.query, cohort.maxRows);
        if (reread.truncated || await resultSummaryHash(reread.rows) !== cohort.hash) {
          throw new Error('Outcome assessment source changed during collection; retry');
        }
    ...
    failureStage = 'window_claim';
    windowClaimClient = base44;
    windowOwnership = await acquireOutcomeWindowClaim(base44.asServiceRole.entities, agencyId, windowKey);
    if (!windowOwnership) return Response.json({ success: false, error: 'An outcome computation owns this window', retry_with_same_key: true }, { status: 409 });
    if (windowOwnership.reconciled) {
      windowOwnership = null;
      return Response.json({ success: false, error: 'Previous outcome publication reconciled; retry the request',
        window_publication_reconciled: true, retry_with_same_key: true }, { status: 409 });
    }
    ...
    const publishedRunValidationError = async (row) => {
      ...
      if (leaseValidationError ||
          ...
          row?.failed_at != null || row?.failure_stage != null ||
          row?.agency_id !== agencyId ||
          row?.run_key !== runKey ||
    ...
    const attemptId = windowOwnership.claim.token;
    const startedAt = windowOwnership.claim.started_at;
    const leaseExpiresAt = windowOwnership.claim.expires_at;
    ...
    // Reject legacy or externally inserted duplicate runs while holding the
    // Agency window claim. Publication separately consumes that existing-row
    // claim; this scan is additional conflict detection, not the lock.
    ...
    const dischargePage = await captureAssessments(dischargeQuery, MAX_DISCHARGE_ROWS);
    ...
    failureStage = 'verify_source_snapshot';
    const sourceSnapshot = await verifySourceSnapshot();
    ...
    // Existing-row window ownership is the exclusion mechanism; this scan
    // additionally rejects historical or externally inserted conflicting runs.
    ...
    failureStage = 'publication_ownership';
    if (!await beginOutcomePublication(base44.asServiceRole.entities, windowOwnership, runId)) {
      throw new Error('Outcome publication ownership was not verified');
    }
    ...
    // final phantom window after this query. The consumed Agency claim above
    // supplies the required existing-row publication fence.
    ...
    console.error('Outcome computation failed at stage:', failureStage);
    ...
    if (activeRunClient && activeRunId && activeRunSnapshot && failureStage !== 'publish_run') {
      try {
        ...
        console.error('Outcome failure transition remains unconfirmed');
      }
    }
    return Response.json({ success: false, error: 'Internal server error' }, { status: 500 });
  } finally {
    if (windowOwnership && !await releaseOutcomeWindowClaim(windowClaimClient.asServiceRole.entities, windowOwnership).catch(() => false)) {
      return Response.json({ success: false, error: 'Outcome publication requires reconciliation', retry_with_same_key: true }, { status: 503 });
    }
  }
});
